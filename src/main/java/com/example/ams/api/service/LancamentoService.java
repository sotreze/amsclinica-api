	...
	public void avisarSobreLancamentosConsultas() {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Preparando envio de "
					+ "e-mails de aviso de lançamentos com datas de consultas para cadastro.");
		}

		List<Lancamento> consultas = lancamentoRepository
				.findByDataConsultaLessThanEqualAndDataConsultaIsNull(LocalDate.now());

		if (consultas.isEmpty()) {
			logger.info("Sem lançamentos com datas de consultas agendadas para aviso.");

			return;
		}

		logger.info("Exitem {} lançamentos de consultas.", consultas.size());

		List<Usuario> destinatarios = usuarioRepository
				.findByPermissoesDescricao(DESTINATARIOS);

		if (destinatarios.isEmpty()) {
			logger.warn("Existem lançamentos de consultas, mas o "
